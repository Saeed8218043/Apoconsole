...
class Menu {
    ...
    */
    ...
    public function build() {
      ...
      if (isset($this->items['warehouses']) && isset($this->items['warehouses']['sub']) && isset($this->items['warehouses']['sub']['items'])){
    //   dd($this->items['warehouses']['sub']['items']);

       if (count($this->items['warehouses']['sub']['items']) == 3){
         array_push($this->items['warehouses']['sub']['items'],
        array("classes" => ['content'=>''],
          "content" => "<hr></hr>"));
      }
      ...
      foreach ($warehouses as $warehouse) {
        $allowedUserIds = explode(',', $warehouse->user_id);

        if (auth()->user()->is_admin() || in_array(auth()->user()->id, $allowedUserIds) || (in_array('warehouses.list.showall', auth()->user()->perms()))) {
            array_push($this->items['Return_warehouses']['sub']['items'], [
