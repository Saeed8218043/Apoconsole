        ...
        if( $_POST['is_bundle'] != "false" ){
            ...
            if (sizeof($_POST['sku___']) > 0) {
                ...
                for ($s = 0; $s < sizeof($_POST['sku___']); $s++) :
                    ...
                    $Get__data = $db->query("SELECT `part_no`, `make_id`, `model_id`, `note` FROM `xml_test` WHERE `part_no` = '" . $temp_sku . "' GROUP BY `xml_test`.`model_id` ");
