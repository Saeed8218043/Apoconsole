...
if (sizeof($GetDB_Shopify_SHOP) > 0) {
    foreach ($GetDB_Shopify_SHOP as $shop__key => $shop__value) {
        ...
        if (isset($_GET['sku_____search'])) {
            if ($_GET['sku_____search'] != "") {
                $SKU_for_update = $_GET['sku_____search'];
            } else {
                echo "error t";
                die();
            }
                        ...
                        if (isset($GRAPHqL_data['data']['products']['edges']) && $GRAPHqL_data['data']['products']['edges'] != array()) {
                            ...
                            foreach ($GOT_GRAPHQL__OUT_ARRAY as $key => $GOT_GRAPHQL__OUT) :
                                ...
                                if (mysqli_num_rows($check_if_sku_exist_in_db) > 0) {
                                    ...
                                    print_r([$access_token, $host_shop, "2022-04", $temp_Parent_varient_product_id, $temp_db_product_netPrice]);
                                    $a = Adject_Price__Function($access_token, $host_shop, "2022-04", $temp_Parent_varient_product_id, $temp_db_product_netPrice);
                                    print_r("\n");
                                    print_r($a);
                                    if (isset($location_ID) && isset($inventory_item_ID) && isset($l)) {
                                        if ($location_ID != ""  &&  $inventory_item_ID != "" ) {
                                            print_r([$access_token, $host_shop, "2022-04", $location_ID, $inventory_item_ID, $l]);
                                            $b = Adject_Quantity__Function($access_token, $host_shop, "2022-04", $location_ID, $inventory_item_ID, $l);
                                             print_r($b);
                                        ...
                                        echo "ferror Updating...";
                                        die();
                                    }

                                    // print_r($res);
                                } else if (0 || mysqli_num_rows($check_if_sku_exist_in_db) == 0) {
                                    ...
                                    $db->query("UPDATE `inventory_prices` SET `location_inventory_id_check`= 1 WHERE `sku` = '$temp_db_product_sku' ");
                                    print_r([$access_token, $host_shop, "2022-04", $temp_Parent_varient_product_id, $temp_db_product_netPrice]);
                                    $b = Adject_Price__Function($access_token, $host_shop, "2022-04", $temp_Parent_varient_product_id, $temp_db_product_netPrice);
                                    print_r("\n");
                                    ...
                                    if (isset($location_ID) && isset($inventory_item_ID) && isset($l)) {
                                        if ($location_ID != ""  &&  $inventory_item_ID != "" && $l != 0) {
                                            ...
                                            echo "aerror Updating...";
                                            die();
                                        }
                                    } else {
                                        echo "berror Updating...";
                                        die();
                                    }
                                }
