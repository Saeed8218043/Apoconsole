...
setInterval(function () {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          if(resp.status == true ){
            ...
            $(".huzi-ac-body p").html(`<b>Fitment Result :: </b><br>`+ resp.data.note +`.`);
          }else{
  ...
  $('#bundle-trigger___id').on("click", function (){
    ...
    for (let sku_i = 0; sku_i < leng; sku_i++) {
      ...
      fetch(window.Shopify.routes.root + "search/suggest.json?q="+ temp_sku +"&resources[type]=product&resources[options][fields]=variants.sku")
      ...
      .then((suggestions) => {
        ...
        if (productSuggestions.length > 0) {
          ...
          temp___tags = firstProductSuggestion.tags;
          // console.log(temp___tags);
          // $('.psgl-sku span').text();
          var str = temp___tags;
          var years___ = null;
          ...
          // console.log(model___);
          for (let index = 0; index < years___.length; index++) {
            $($(".fitment_year_")[sku_i]).append('<option value="'+ years___[index] +'">'+ years___[index] +'</option>');                    
          }
          for (let index = 0; index < model___.length; index++) {
            $($(".fitment_model_")[sku_i]).append('<option value="'+ model___[index] +'">'+ model___[index] +'</option>');                    
          }
        }
  ...
  $(".test-fitment_test____").on('click', function() {
    ...
    if (sku_____ != "" && year_____ != "" && model_____ != "") {
