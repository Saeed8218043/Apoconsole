                ...
                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                    ...
                    <div class="menu menu-sub menu-sub-dropdown w-300px w-md-325px" data-kt-menu="true">
                        ...
                        <div class="px-7 py-5" data-kt-user-table-filter="form">
                            <!--begin::Input group-->
                            <div class="mb-10">
                                <label class="form-label fs-6 fw-bold">Vendor:</label>
                                <select id="vendor_f"   class="form-select form-select-solid fw-bolder" data-kt-select2="true"
                    ...
                    <!--end::Filter-->
                     ...
                     {{ theme()->getView('pages/warehouses/add',['users'=>$users]) }}
                      {{ theme()->getView('pages/warehouses/edit',['users'=>$users]) }}
        ...
        <script id="usertable">
            ...
            var KTUsersList = (function() {
                ...
                var initUserTable = function() {
                    ...
                    datatable = $(table).DataTable({
                        ajax: {
                            url: "<?= route('warehouses.list.datatableapi') ?>",
                            data: {
                                found: 1,
                                // ;
                                ...
                                if (result.value) {
                                    Swal.fire({
                                        ...
                                        .then(function() {
                                            ...
                                            $.ajax({
                                                url: "<?= route('warehouses.list.bulkdelete') ?>",
                                                type: 'POST',
                                                data: {
                                                    "list": id,
                                                    "_token": token,
            ...
            function edit(id){
               ...
               console.log(data);
                   ...
                   // Split the emails by <br> and trim any extra whitespace
             var emails = data.user.split(/<br\s*\/?>/).map(email => email.trim());

            // Get the select element
            var selectElement = document.querySelector('.select-user');

            // Set the selected options
            Array.from(selectElement.options).forEach(option => {
                if (emails.includes(option.text)) {
...
// Optionally, you might need to trigger a change event on the select element
// to ensure any JavaScript libraries or plugins that depend on this interaction are updated.
var event = new Event('change', { bubbles: true });
selectElement.dispatchEvent(event);

               $('#kt_modal_edit_coupon_form').find('input[name=name]').val(data['name']);
                $('#kt_modal_edit_coupon_form').find('select[name=user]').val(data['user_id']);
