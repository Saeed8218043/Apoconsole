            ...
            <div class="card-title">
                <!--begin::Search-->
                 <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <!--end::Svg Icon-->
                    <input type="text" id="order_search" onkeypress="order_search(event)" class="form-control form-control-solid w-250px ps-14" placeholder="Search Order" />
                </div>
                <!--end::Search-->
            </div>
            <!--begin::Card title-->
            ...
            <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_coupons">
                ...
                <thead>
                    ...
                    <tr class="text-start text-muted fw-bolder fs-7 text-uppercase gs-0">
                        ...
                        <th class="min-w-25px w-25px">ID</th>
                        <th class="min-w-25px">Date</th>
                        <th class="min-w-50px">PO Number</th>
...
<script>
 //const resGet = axios.get("{{ route('trq_orders') }}").then(function (res){

     //table(res.data.data._embedded.orders);
//  });

 function table(data){
            console.log(data);
            $('#kt_table_coupons').DataTable({
            data: data,
            "columns":[
                {     "data"     :     "id"},
                {     "data"     :     "poNumber"},
        ...
        function order_search(e){
            value=document.getElementById("order_search").value;
            if(e.keyCode === 13){
     ...
     function detail(id){
               ...
               orderdetail = '<strong>Created: </strong>'+data['_date'];
               var track = '';
                    var tracking = '';
                    if (data.items.length > 0) {
                        for (var i = 0; i < data.items.length; i++) {
                            if (data.items[i].trackingNumbers && data.items[i].trackingNumbers[0] != undefined) {
                                var trackingNumber = data.items[i].trackingNumbers[0].trackingNumber;
                                if (trackingNumber) {
                                    track = '<a target="_blank" href="https://wwwapps.ups.com/WebTracking/track?track=yes&trackNums=' + trackingNumber + '" >' + trackingNumber + '</a>';
                                    tracking = '<br><strong>Tracking Number: </strong>' + track + '<br>';
                                    break; // Exit the loop once a valid tracking number is found
                                }
                            }
                        }
                    }
               ...
               $('.shipping_detail').html('');
               if(data.invoices[0]){
                   const date = new Date(data.invoices[0].orderDate);
                   const formattedDate = moment(date).format('DD-MM-YYYY');
                   $('.shipping_detail').append('<tr><td scope="col">'+data.invoices[0].erpOrderId+'</td><td scope="col">'+formattedDate+'</td><td scope="col">'+data.invoices[0].orderTotal+'</td><td></td></tr>');
               }
               if(data.rmas[0]){
                    $('.shipping_detail').append('<tr><td scope="col">'+data.rmas[0].erpOrderId+'</td><td scope="col"></td><td scope="col">'+data.rmas[0].orderTotal+'</td><td></td></tr>');
                }
            ...
            //   $('#kt_modal_detail_coupon_form').find('#trq_id').html(data['trq_id']);
            ...
            //     $('#kt_modal_detail_coupon_form').find('#vendor').html(data['vendor']);
            //     $('#kt_modal_detail_coupon_form').find('#variant').html(data['variant']);
